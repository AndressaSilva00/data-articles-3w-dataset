...
Each operational status is defined according to valve configurations and the nature of recent or ongoing operations. In summary, we have the following:

- Open: all production valves (M1, W1, SDV-P, and PCK) are open; auxiliary valves (PXO, and XO) are closed;
...
* Regarding labeling:
    1. Historical events are mapped in a Tracking System, which ensures accessibility and traceability for labelers (experts from Petrobras);
...
* Petróleo Brasileiro S.A. (Petrobras), Rio de Janeiro, Brazil
    * Ricardo Emanuel Vaz Vargas, Jean Carlos Dias de Araújo & Mateus de Araujo Fernandes
